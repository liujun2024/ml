...
if __name__ == '__main__':
    ...
    # a = np.array_split(df, 2)
    # # print(type(a[0]))
    # exit(0)

    # # 切分数据集
    # df_train_bins, df_test_bins = utils.split_df_random(
    ...
    # 计算shap值
    # model_rfr.calculate_shap(cpu=4)

    # 计算shap interaciton values
    model_rfr.calculate_shap_interaction(cpu=4)

    # explainer = shap.Explainer(model_rfr.model)
    # explaination = explainer(model_rfr.df_train.iloc[:, :-1])
    # print(explaination)
    # shap.plots.scatter(explaination[:, 'f2'], color=explaination[:, 'f5'])

    # 读取shap interaction values
    # model_rfr.plot_shap_interaction(show=True, dpi=100)
