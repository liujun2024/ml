...
class ExtremeGradientBoosting:
    ...
    def __init__(self,  path_h5: Path, cv=5, cpu=4):
        ...
        self.dict_params_init = {
            ...
            # 'max_leaves': [0, 10, 50, 100, 200, 500],
    ...
    def __load_model(self):
        ...
        # 载入模型
        self.model = sio.load(self.path_model, trusted=['xgboost.core.Booster', 'xgboost.sklearn.XGBRegressor'])
    
        # 预测训练集和测试集
        self.__predict()

    def fit(self, overwriting=False):

        # 判断模型是否已经训练完毕
        if self.model is not None and not overwriting:
            print('模型已存在，跳过！')
            return
    ...
    def plot_lc(self, show=False):
        ...
        # 获取作图顺序
        # self.h5rw.read_performance()

        # 作图顺序
        list_order = list(self.dict_params_all.keys())
            # self.h5rw.dict_model[f'{suffix_kw}']['sequence_for_plot']
        # [f'{suffix_kw}/hyperparameters'].attrs['sequence_for_plot']
        ...
        # for i, (k, v) in enumerate(self.dict_params_all.items()):
...
class XGBoost(ShapBasedExplainer):
    """ XGBoost模型, 继承自ShapBasedExplainer, 用于替代ExtremeGradientBoosting类 """
