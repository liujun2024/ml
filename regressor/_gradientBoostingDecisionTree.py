""" 梯度提升决策树算法（GradientBoostingDecisionTree）"""
...
class GradientBoostingDecisionTree:
    ...
    def __create_model(self, dict_param: dict):
        ...
        model = GradientBoostingRegressor(
            loss='squared_error',
            # learning_rate= dict_param['learning_rate'],
            ...
            # min_weight_fraction_leaf= dict_param['min_weight_fraction_leaf'],
    ...
    def __predict(self):
        ...
        # 训练集模型表现
        self.r2_train, self.rmse_train, self.y_predict_train = map(dict_predict_train.get, ['r2', 'rmse', 'predict'])

        # 预测测试集
        dict_predict_test = utils.predict(model=self.model, x=self.x_test, y=self.y_test)

        self.r2_test, self.rmse_test, self.y_predict_test = map(dict_predict_test.get, ['r2', 'rmse', 'predict'])
    ...
    def plot_performance(self, show=False):
        ...
        plot.performance_scatter(
            data_={'r2': self.r2_train, 'rmse': self.rmse_train, 'df': self.df_predict_train},
            annotation_='Training',
            ax=ax['b'],
        )
        
        plot.performance_scatter(
            data_={'r2': self.r2_test, 'rmse': self.rmse_test, 'df': self.df_predict_test},
            annotation_='Test',
            ax=ax['d'],
        )
...
class HistGradientBoostingDecisionTree(GradientBoostingDecisionTree):
    """ HistGradientBoostingDecisionTree """

    def __init__(self, x_train: pd.DataFrame | np.ndarray, y_train: pd.Series | np.ndarray, cv=5, cpu=4):
        super().__init__(x_train, y_train, cv, cpu)
        
        # 需要拟合的参数
        self.list_params_for_fitting = ['max_depth', 'max_bins']
    ...
    def __create_model(self, dict_param: dict):
        ...
        model = HistGradientBoostingRegressor(
            loss='squared_error' if 'loss' not in dict_param.keys() else dict_param['loss'],
            # learning_rate= dict_param['learning_rate'],
            max_iter=1000 if 'max_iter' not in dict_param.keys() else dict_param['max_iter'],
            max_leaf_nodes=31 if 'max_leaf_nodes' not in dict_param.keys() else dict_param['max_leaf_nodes'],
            max_depth= dict_param['max_depth'],
            min_samples_leaf=20 if 'min_samples_leaf' not in dict_param.keys() else dict_param['min_samples_leaf'],
            max_bins=255 if 'max_bins' not in dict_param.keys() else dict_param['max_bins'],
            early_stopping=True,
            random_state=42,
            max_features=1.0 if 'max_features' not in dict_param.keys() else dict_param['max_features'],
            verbose=0,
        )
